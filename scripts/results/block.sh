...
for index in ECOLI SENTERICA HUMANGUT; do
	...
	for size in {16..26}; do
		...
		for i in {0..6}; do
			...
			OUTPUT=$TMP_DIR/block_${index}_${twopower}_${i}
			OUTPUT_NO_REORDER=$TMP_DIR/block_${index}_${twopower}_no_reorder_${i}

			export MEASURED_LOGFILE="$LOG_DIR/${index}_${twopower}_${i}.txt"
	        export MEASURED_USAGEFILE="$USG_DIR/${index}_${twopower}_${i}.txt"
			monitor_cmd $TOOL -i $INPUT -z $OUTPUT -c $SAMPLES --header 49 -s 10000 -b $twopower --wlog $size -f $ORDER -j "$MTC_DIR/metrics_${index}_${twopower}_${i}.json" --config-path $CONFIG

			export MEASURED_LOGFILE="$LOG_DIR/${index}_${twopower}_no_reorder_${i}.txt"
	        export MEASURED_USAGEFILE="$USG_DIR/${index}_${twopower}_no_reorder_${i}.txt"
			monitor_cmd $TOOL -i $INPUT -z $OUTPUT_NO_REORDER -c $SAMPLES --header 49 -s 10000 -b $twopower --wlog $size -n -j "$MTC_DIR/metrics_${index}_${twopower}_no_reorder_${i}.json" --config-path $CONFIG
