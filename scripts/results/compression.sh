...
for index in ECOLI SENTERICA HUMANGUT; do
	...
	$KMCOMP_DIR/no_dm_vptree/build/kmcomp -i $TMP_DIR/${index}_ref.cmbf -c $SAMPLES -b 65536 --header 49 -z $TMP_DIR/temp -t $ORDER --config-path $CONFIG >/dev/null
	echo "\$\$\$ $KMCOMP_DIR/no_dm_vptree/build/kmcomp -i $TMP_DIR/${index}_ref.cmbf -c $SAMPLES -b 65536 --header 49 -z $TMP_DIR/temp -t $ORDER --config-path $CONFIG >/dev/null"
	...
	INPUT_R=$RUN_DIR/matrices/reordered/$REF_MATRIX
	...
	monitor_cmd $TOOL -i $INPUT_R -z $OUTPUT -c $SAMPLES --header 49 -b 65536 -s 10000 -n -j "$MTC_DIR/metrics_${index}_ref.json" --config-path $CONFIG

	export MEASURED_LOGFILE="$LOG_DIR/${index}_no_reorder_ref.txt"
    export MEASURED_USAGEFILE="$USG_DIR/${index}_no_reorder_ref.txt"
	monitor_cmd $TOOL -i $INPUT -z $OUTPUT_NO_REORDER -c $SAMPLES --header 49 -b 65536 -s 10000 -n -j "$MTC_DIR/metrics_${index}_no_reorder_ref.json" --config-path $CONFIG
	...
	for i in {0..6}; do
		echo -e "\t\tMatrix $i"
		INPUT_R=$RUN_DIR/matrices/reordered/matrix_${i}.cmbf
		...
		monitor_cmd $TOOL -i $INPUT_R -z $OUTPUT -c $SAMPLES --header 49 -b 65536 -s 10000 -n -j "$MTC_DIR/metrics_${index}_${i}.json" --config-path $CONFIG

		export MEASURED_LOGFILE="$LOG_DIR/${index}_no_reorder_${i}.txt"
        export MEASURED_USAGEFILE="$USG_DIR/${index}_no_reorder_${i}.txt"
		monitor_cmd $TOOL -i $INPUT -z $OUTPUT_NO_REORDER -c $SAMPLES --header 49 -b 65536 -s 10000 -n -j "$MTC_DIR/metrics_${index}_no_reorder_${i}.json" --config-path $CONFIG
